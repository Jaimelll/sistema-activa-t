-- Database Schema Strict (Integer IDs for Master Tables)
...
drop table if exists public.becas cascade; -- Ensuring becas is also dropped
...
-- 2. Parametric Tables (INTEGER IDs explicitly)
create table public.ejes (
  id integer primary key, -- Derived from 'Numero'
  descripcion text,
  fase text
);
...
create table public.regiones (
  id integer primary key, -- Derived from 'Numero'
  descripcion text unique -- Canonical name (e.g. JUNIN)
);

create table public.etapas (
  id integer primary key, -- Derived from 'Numero' (if available) or specific mapping
...
create table public.instituciones_ejecutoras (
  id uuid primary key default uuid_generate_v4(), -- Institutions might not have 'Numero', keep UUID or use AutoInc
  nombre text unique,
  ruc text,
  correo text,
